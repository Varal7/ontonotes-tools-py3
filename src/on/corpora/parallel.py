...
class parallel_document(object):
    ...
    def copy_to_different_trees(self, original_from_to, translation_from_to,
                                to_original_document, from_original_document,
                                to_translation_document, from_translation_document,
                                to_bank, existing_to_document=None):

        assert existing_to_document is None

        to_parallel_document = parallel_document(to_original_document.document_id, to_translation_document.document_id, to_bank.extension)
        to_parallel_document.set_tree_documents(to_original_document, to_translation_document)

        o_root_to_root = on.corpora.tree.treebank.build_root_to_root(original_from_to)
        t_root_to_root = on.corpora.tree.treebank.build_root_to_root(translation_from_to)

        for from_sentence in self:
            tree_originals = o_root_to_root[from_sentence.tree_original]
            tree_translations = t_root_to_root[from_sentence.tree_translation]

            for tree_original in tree_originals:
                for tree_translation in tree_translations:
                    to_sentence = parallel_sentence(tree_original.id, tree_translation.id)
                    to_sentence.set_trees(tree_original, tree_translation)
                    to_parallel_document.append(to_sentence)

        return to_parallel_document
...
class parallel_bank(abstract_bank):
    ...
    def copy_to_different_trees(self, alignments_from_to, alignments_to_from,
                                to_treebank, from_treebank):
        """ you want copy_to_different_trees_parallel """
        raise Exception("use copy_to_different_trees_parallel")

    def copy_to_different_trees_parallel(self, alignments_from_to, alignments_to_from,
                                         to_original_treebank, from_original_treebank,
                                         to_translation_treebank, from_translation_treebank):
        ...
        for from_parallel_document in self:
            from_original_document = from_parallel_document.tree_document_original
            from_translation_document = from_parallel_document.tree_document_translation

            to_original_document = to_original_treebank.get_document(from_original_document)
            to_translation_document = to_translation_treebank.get_document(from_translation_document)

            to_bank.append(from_parallel_document.copy_to_different_trees(
                alignments_from_to[from_original_document.document_id],
                alignments_from_to[from_translation_document.document_id],
                to_original_document, from_original_document,
                to_translation_document, from_translation_document, to_bank))

        to_original_treebank.inform_enriched(to_bank)
        to_translation_treebank.inform_enriched(to_bank)
