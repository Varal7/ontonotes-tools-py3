...
PREGOLD_STATUSES=["QUEUE", "DOUBLE", "SINGLE", "PREGOLD"]
...
class argument_type(on.corpora.abstract_type_table):
    ...
    allowed = [ "ARG0",
                ...
                "ARGM-PRX", # predicating expression
...
class abstract_analogue(abstract_holder):
    ...
    def coreferential_non_coindexed_nodes(self):
        ...
        not_pointing = [] # these are subtrees that don't point at other subtrees we already have

        if self.analogue_type == "link":
            pass # allowed in links
        elif not all(len(a_argument) == 1 for a_argument in self):
            pass # comma separated cases are tricky and rare, call them all ok
        else:
            # we know that a_argument[0] is all the subtrees because
            # we just made sure of the length
            subtrees = set(move_down(a_argument[0].subtree) for a_argument in self)

            for a_subtree in subtrees:
                if a_subtree and move_down(a_subtree.identity_subtree) not in subtrees:
                    not_pointing.append(a_subtree)
...
class abstract_node(abstract_proposition_bit):
    ...
    def copy_to_different_trees(self, alignment_from_to, a_parent):
        """ see :meth:`on.corpora.proposition.proposition.copy_to_different_trees` """

        to_node = self.__class__(None, None, None, a_parent)
        to_node.primary = self.primary

        # if true, we could have been lower but didn't choose to be
        prefer_down = not self.subtree.children

        to_leaves = [to_leaf for from_leaf in self.subtree.leaves()
                     for to_leaf in alignment_from_to.get(from_leaf, [])]

        def adderr(errcode, *comments):
            comments = list(comments)
            comments.insert(0, "Node id: %s" % to_node.node_id)
            to_node.errcomms.append([errcode, comments])

        if not to_leaves:
            if self.subtree.is_leaf() and self.subtree.is_trace():
                errcode = "notracetarget"
            else:
                errcode = "notarget"
            adderr(errcode, "from_subtree: %s" % " ".join(a_leaf.word for a_leaf in self.subtree))
            return to_node
        ...
        for a_leaf in to_leaves:
            if not a_tree:
                a_tree = a_leaf.get_root()
            else:
                if a_tree != a_leaf.get_root():
                    adderr("spanstrees")
                    return to_node

            if first_leaf is None or a_leaf.get_token_index() < first_leaf.get_token_index():
                first_leaf = a_leaf
            if last_leaf is None or a_leaf.get_token_index() > last_leaf.get_token_index():
                last_leaf = a_leaf

        new_first_leaf, new_last_leaf = a_tree.get_closest_aligning_span(first_leaf, last_leaf,
                                                                         ignore_traces=True,
                                                                         ignore_punct=True,
                                                                         ignore_determiners=False,
                                                                         ignore_possessives=False)

        if new_first_leaf and new_last_leaf:
        ...
        new_string = " ".join(a_leaf.get_word() for a_leaf in a_subtree)
        old_string = " ".join(a_leaf.get_word() for a_leaf in self.subtree)

        trace_errcomms = self._find_trace_issues(self.subtree, a_subtree, alignment_from_to)
        if trace_errcomms:
            to_node.errcomms += trace_errcomms
        elif new_string != old_string:
            adderr("modtok" if same_except_for_tokenization_and_hyphenization(new_string, old_string) else "leafdiff",
                   "from string: %s" % old_string,
                   "to string: %s" % new_string,
                   "from_subtree: %s" % " ".join(a_leaf.word for a_leaf in self.subtree),
                   "to_subtree: %s" % " ".join(a_leaf.word for a_leaf in a_subtree))

        to_node.subtree = a_subtree

        #on.common.log.status("          > ", to_node.enc_self)
...
class predicate(abstract_node_holder):
    ...
    def copy_to_different_trees(self, alignment_from_to, a_predicate_analogue):
        """ see :meth:`on.corpora.proposition.proposition.copy_to_different_trees` """

        to_predicate = predicate("", None, None, a_predicate_analogue)

        for from_predicate_node in self:
            to_predicate_node = from_predicate_node.copy_to_different_trees(alignment_from_to, to_predicate)
...
class predicate_analogue(abstract_analogue):
    ...
    def copy_to_different_trees(self, alignment_from_to, a_proposition, analogues_from_to):
        """ see :meth:`on.corpora.proposition.proposition.copy_to_different_trees` """

        analogues_from_to[self] = predicate_analogue("", self.type, None, None, a_proposition)

        for from_predicate in self:

            #on.common.log.status("      > ", from_predicate.enc_self)
            to_predicate = from_predicate.copy_to_different_trees(alignment_from_to, analogues_from_to[self])
            #on.common.log.status("     -> ", to_predicate.enc_self)

        return analogues_from_to[self]
...
class link(abstract_node_holder):
    ...
    def copy_to_different_trees(self, alignment_from_to, a_link_analogue):
        """ see :meth:`on.corpora.proposition.proposition.copy_to_different_trees` """

        to_link = link("", a_link_analogue)

        for from_link_node in self:
            from_link_node.copy_to_different_trees(alignment_from_to, to_link)
...
class link_analogue(abstract_analogue):
    ...
    def copy_to_different_trees(self, alignment_from_to, a_proposition, analogues_from_to):
        ...
        analogues_from_to[self] = link_analogue("", self.type, a_proposition, analogues_from_to[self.associated_argument])

        for from_link in self:
            to_link = from_link.copy_to_different_trees(alignment_from_to, analogues_from_to[self])
...
class argument(abstract_node_holder):
    ...
    def copy_to_different_trees(self, alignment_from_to, a_argument):
        """ see :meth:`on.corpora.proposition.proposition.copy_to_different_trees` """

        to_argument = argument("", a_argument)

        for from_argument_node in self:
            from_argument_node.copy_to_different_trees(alignment_from_to, to_argument)
...
class argument_analogue(abstract_analogue):
    ...
    def __init__(self, enc_argument_analogue, a_proposition):
        ...
        if enc_argument_analogue:
            ...
            elif type_bits[0] == "C" or type_bits[0] == "R":
                """ this is also a legitimate argument C- or R-"""
                #self.type = "-".join(type_bits)
    ...
    def copy_to_different_trees(self, alignment_from_to, a_proposition, analogues_from_to):
        ...
        analogues_from_to[self] = argument_analogue("", a_proposition)
        analogues_from_to[self].type = self.type

        for from_argument in self:
            from_argument.copy_to_different_trees(alignment_from_to, analogues_from_to[self])
...
class proposition(object):
    ...
    Attributes:
      ...
      .. attribute:: quality
         ...
         pregold
           double annotated, adjudicated, propbank-internal format
    ...
    Methods:
      ...
      .. automethod:: copy_to_different_trees
      .. automethod:: __getitem__
      .. automethod:: get_argument_tagged_string
    ...
    def __init__(self, encoded_prop, subcorpus_id, document_id, a_proposition_bank=None, tag=None):
        ...
        if encoded_prop:
            ...
            if "." in lemma_and_sense:
                ...
                if self.pb_sense_num == "XX" or self.pb_sense_num == "LV":
                    pass
                else:
                    try:
                ...
                if not found_connection_flag:
                    ...
                    for a_node_pointer in enc_link_node_pointers.split("*"):
                        if(a_node_pointer.endswith(":0")):
                            b_node_pointer = a_node_pointer.replace(":0", ":1")
                        elif(a_node_pointer.endswith(":1")):
                            b_node_pointer = a_node_pointer.replace(":1", ":0")
                        else:
                            b_node_pointer = a_node_pointer
    ...
    def get_enc_prop_list(self, short_args_form=False, no_links_form=False):
        ...
        def find_argument_to_use(a_prop, a_argument_analogue):
            ...
            if len(a_argument_analogue.coreferential_non_coindexed_nodes()) > 1:
                return None, None
            ...
            for a_argument in a_argument_analogue:
                ...
                sys.stderr.write("len(a_argument)%s\n" % (len(a_argument)))
                if len(a_argument) != 1: sys.stderr.write("WARNING: ,* argument: %s\n" % (a_argument))
                if len(a_argument) != 1: sys.stderr.write("WARNING: ,* argument enc: %s\n" % (a_argument.enc_self))
        ...
        for a_thing in self:
                                        ...
                                        if b_link_node is not a_link_node:
                                            if b_link_node.subtree:
                                                if nid(b_link_node.subtree) not in already_linked_nodes:
                                                    ...
                                                    on.common.log.info("    Changing enc_arg from %s to %s" % (enc_arg, new_enc_arg))
                                                    enc_arg = new_enc_arg
            ...
            if not short_args_form and self.get_tree() and a_thing.analogue_type != "link":
                ...
                for node_to_add in all_referenced_nodes - all_existing_nodes:
                    #on.common.log.info("    Adding %s to enc_arg: %s" % (nid(node_to_add), enc_arg))
    ...
    def is_pregold(self):
        return self.quality.upper() in PREGOLD_STATUSES


    def get_argument_tagged_string(self):
        """ produce a sgml representation of this proposition

        For example, the proposition with pointer::

          nw/wsj/12/wsj_1203@1203@wsj@nw@en@on 9 3 gold fall-v fall.01

        has the argument string::

          3:0-rel 1:1-ARG1 4:1-ARG4 7:1-ARGM-TMP 18:0*0:1-ARGM-TMP

        This translates into an argument tagged string of::

          DOMAIN/FRAME/fall.01.v.ar: <S TPOS="3@9@nw/wsj/12/wsj_1203@1203@wsj@nw@en@on">
        ...
        There are no newlines in the argument tagged string; the ones
        in the example are added for readability.

        The basic format is that ''<S>...</S>'' sets off the sentence
        ...
        of a ''TARGET'' is always ''y''.

        The value of the ''FE'' attribute is normally just the name of
        the node, as in 'ARG0'.  In the cases where this would create
        multiple arguments, non 'primary' arguments get 'R-' and 'C-'
        prefixes.

        Links are not included in this representation.  or, more
        correctly, the propositions should be of the no_link form
        before this method is used.

        """

        def annotate_string(tag, attr_name, attr_val, start, end):
            """ start is inclusive, end is exclusive """
            a_token_list[start] = '<%s %s="%s"> %s' % (tag, attr_name, attr_val, a_token_list[start])
            a_token_list[end-1] = '%s </%s>' % (a_token_list[end-1], tag)


        a_subtree = self.get_primary_predicate().subtree
        a_tree = a_subtree.get_root()
        a_token_list = [x.get_word() for x in a_tree]
        a_leaves = list(a_tree.leaves())

        # annotate the predicate
        annotate_string('C', 'TARGET', 'y', a_subtree.start, a_subtree.end)


        for a_argument_analogue in self.argument_analogues:

            #print "="*80
            #print a_argument_analogue

            core_subtree, r_subtrees, c_subtrees = a_argument_analogue.get_subtree_tuple()
            #print "-"*80
            ...
            if not core_subtree:
                """ don't include argument analogues where all the nodes are traces."""
                continue

            for a_subtree in [core_subtree] + r_subtrees + c_subtrees:
                if a_subtree in r_subtrees:
                    prefix = "R"
                elif a_subtree in c_subtrees:
                    prefix = "C"
                else:
                    assert a_subtree is core_subtree
                    prefix = ""

                FE_attribute = a_argument_analogue.type
                if prefix:
                    FE_attribute = "%s-%s" % (prefix, FE_attribute)

                annotate_string('C', 'FE', FE_attribute, a_subtree.start, a_subtree.end)



        token_index, rest = self.id.split("@", 1)
        token_index = int(token_index)

        word_index = a_leaves[token_index].get_word_index()
        p2s_id="%s@%s" % (word_index, rest)

        #annotate_string('S', 'TPOS', "%s -> %s" % (self.id, p2s_id), 0, len(a_token_list))
        annotate_string('S', 'TPOS', p2s_id, 0, len(a_token_list))
        return 'DOMAIN/FRAME/%s.%s.v.ar: %s' % (self.lemma, self.pb_sense_num, " ".join(a_token_list))


    def get_argument_tagged_conll_string(self):
        """ produce a conll representation of this proposition
        ...
        def annotate_string(attr_val, start, end):
            """ start is inclusive, end is exclusive """
            a_token_list[start] = '(%s%s' % (attr_val, a_token_list[start])
            a_token_list[end-1] = '%s)' % (a_token_list[end-1])


        a_subtree = self.get_primary_predicate().subtree
        a_tree = a_subtree.get_root()
        a_token_list = ["*" for x in a_tree]
        #a_token_list = [x.word for x in a_tree]

        # annotate the predicate
        annotate_string("V", a_subtree.start, a_subtree.end)
        ...
        for a_argument_analogue in self.argument_analogues:
            ...
            for a_subtree in [core_subtree] + r_subtrees + c_subtrees:
                ...
                annotate_string(FE_attribute, a_subtree.start, a_subtree.end)

        return " ".join(a_token_list)


    def copy_to_different_trees(self, alignment_from_to, subcorpus_id, document_id, tag="gold"):
        """ create a duplicate of this proposition that is aligned with the ``to`` trees in ``alignment_from_to`` """

        to_proposition = proposition("", subcorpus_id, document_id, a_proposition_bank=None, tag=tag)
        to_proposition.lemma = self.lemma
        to_proposition.pb_sense_num = self.pb_sense_num
        to_proposition.original_enc_prop = self.original_enc_prop
        ...
        analogues_from_to = {}
        for from_analogue in self:
            to_analogue = from_analogue.copy_to_different_trees(alignment_from_to, to_proposition, analogues_from_to)

        to_tree = to_proposition.get_tree()
        from_tree = self.get_tree()

        if to_tree and from_tree:
            errcomms = abstract_node._find_trace_issues(from_tree, to_tree, alignment_from_to)
            errcomms = [["prop_%s"%errcode, comments] for errcode, comments in errcomms]
            if errcomms:
                to_proposition.errcomms += errcomms



        return to_proposition
    ...
    def coreferential_non_coindexed_nodes(self):
        """ if A*B does not have A coreferential with B and we are not in a link, that's bad """

        for a_analogue in self:
            not_pointing = a_analogue.coreferential_non_coindexed_nodes()
            if len(not_pointing) > 1:
                return not_pointing
        ...
        def get_arg_start_ends(a_tree):
            ...
            for a_analogue in self:
                if a_analogue.analogue_type == "link":
                    continue
                if self.predicate.type == "m" and (a_analogue.analogue_type == "predicate" or
                                                   a_analogue.type.lower() == "rel-sup"):

                    """ nombank propositions are allowed to have their predicate overlap """
                    continue


                #
                # check if the predicate has a PRN parent and if so, continue
                #

                #print self.predicate
                b_tree = self.get_primary_predicate().subtree.get_root()
                b_leaves = b_tree.leaves()
                i=self.predicate.enc_self.split(":")[0]

                a_parent = b_leaves[int(i)].parent
                while a_parent.parent != None:
                    if(a_parent.phrase_type == "PRN"):
                        self.predicate_in_prn = True
                        #print a_parent
                        break
...
class proposition_document:
    ...
    def write_to_db(self, a_cursor):
        for a_proposition in self:
            if a_proposition.valid:
                if not a_proposition.is_pregold():
                    a_proposition.write_to_db(a_cursor)

    def dump_view(self, a_cursor=None, out_dir="", prop2sgml=False,
                  short_args_form=True, no_links_form=None):
        """ write out to a file

        if prop2sgml, dump in an sgml format
        ...
        with codecs.open(on.common.util.output_file_name(self.document_id, ext, out_dir), "w", "utf-8") as f:
            if prop2sgml:
                if not self.tree_document:
                    raise Exception("proposition_document.dump_view only supports prop2sgml usage after enrichment")
                for a_tree in self.tree_document:
                    for a_leaf in a_tree:
                        if a_leaf.proposition and a_leaf.proposition.valid:
                            f.write(a_leaf.proposition.get_argument_tagged_string() + "\n")

                            tree_string = a_tree.to_string()
                            if False:
                                f.write(a_leaf.proposition.enc_prop + "\n")
                                tree_string = a_tree.pretty_print_tree_string(tree_string)
    ...
    def copy_to_different_trees(self, alignment_from_to, alignment_to_from,
                                from_tree_document, to_tree_document,
                                to_bank, existing_to_document=None,
                                ignore_errors=False):

        if existing_to_document is not None:
            to_proposition_document = existing_to_document
        else:
            to_proposition_document = proposition_document(to_tree_document.document_id, to_bank.extension)

        for from_proposition in self:
            if not from_proposition.valid:
                continue

            to_proposition = from_proposition.copy_to_different_trees(alignment_from_to, to_bank.subcorpus.id,
                                                                      to_tree_document.document_id, tag=to_tree_document.tag)

            if to_proposition.valid:
                to_bank.check_proposition(to_proposition, mode="post-copy",
                                          document_id=from_tree_document.document_id, ignore_errors=ignore_errors)

            orig_enc_prop = from_proposition.original_enc_prop
            if to_proposition.valid:
                to_proposition_document.append(to_proposition)

                path_enc_prop = to_proposition.path_enc_prop

                orig_enc_prop_pointer, orig_enc_prop_args = orig_enc_prop.split(" ----- ")
                path_enc_prop_pointer, path_enc_prop_args = path_enc_prop.split(" ----- ")

                path_enc_prop_args = " ".join(on.common.util.make_look_like(orig_enc_prop_args.split(), path_enc_prop_args.split()))

                path_enc_prop = "%s ----- %s" % (path_enc_prop_pointer, path_enc_prop_args)

                from_tree_str = from_proposition.get_primary_predicate().subtree.get_root().pretty_print()
                to_tree_str = to_proposition.get_primary_predicate().subtree.get_root().pretty_print()

                if from_tree_str == to_tree_str:
                    from_args = from_proposition.enc_prop.split(" ----- ")[-1]
                    to_args = to_proposition.enc_prop.split(" ----- ")[-1]

                    if from_args != to_args:
                        on.common.log.report("proposition", "copying on plain tree changed something",
                                             from_args=from_args,
                                             to_args=to_args,
                                             from_tree_str=from_tree_str,
                                             to_tree_str=to_tree_str)

                identical = from_tree_str == to_tree_str

                if not ignore_errors:
                    on.common.log.adjust(["docid", from_tree_document.document_id, "prop"],
                                         #"prop" if not identical else "universal",
                                         #[["copyok" if not identical else "ok", [] ]],
                                         "universal",
                                         [["ok", [] ]],
                                         orig_enc_prop, path_enc_prop)

            elif False:
                from_tree = from_proposition.get_primary_predicate().subtree.get_root()
                to_trees = set(to_leaf.get_root()
                               for from_leaf in from_tree
                               for to_leaf in alignment_from_to.get(from_leaf, []))

                mappings = []
                for a_from_leaf in from_tree:
                    mappings.append("%s -> %s" % (a_from_leaf.word, ", ".join(t_leaf.word for t_leaf in alignment_from_to.get(a_from_leaf, []) if t_leaf)))

                on.common.log.bad_data_long(from_tree_document.language, "prop", "failed to copy to new trees",
                                            ["data_pointer", orig_enc_prop],
                                            ["from_onf", from_tree.onf()],
                                            ["to_onfs", "\n\n".join(to_tree.onf() for to_tree in to_trees)],
                                            ["mappings", "\n".join(mappings)])

        return to_proposition_document
