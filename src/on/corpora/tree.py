...
class phrase_type:
    ...
    allowed = [ "ADJP",
                ...
                "INC", # incomplete
                "DFL", # disfluency
                "SKIP", # non-speech elements in transcription
                "OTH", # other, similar to ETC pos tag, but when the pos tag is not ETC

                # similar to english EMBED
                "MBD"
                ]
...
class pos_type:
    ...
    allowed = { "AFX":  "AFX",
               ...
               "INF":   None,
                ...
                "URL": "ADD", # URLs and email addresses
...
class tree(object):
    ...
    def bad_data(self, reason, *args, **kwargs):
        ...
        try:
            ...
            on.common.log.warning("could not find language, so not recording the bad_data")
            return
    ...
    def delete_trace_and_code_and_edits(self):
        import copy
        ...
        while(need_to_delete_some_node == True):
            need_to_delete_some_node = False
            ...
            for a_subtree in a_tree.subtrees():
                ...
                while(i<len(a_subtree.children)):
                    if(a_subtree.children[i].tag == "-NONE-"
                       or a_subtree.children[i].tag == "CODE"
                       or a_subtree.children[i].tag == "EDITED"):
            ...
            for a_subtree in a_tree.subtrees():
                ...
                if(len(a_subtree.children) == 0 and (a_subtree.is_a_leaf == False)):
                    a_subtree.marked_for_deletion = True
                    need_to_delete_some_node = True
            ...
            if(iteration > 5):
                sys.stderr.write("original tree: %s\n" % (self))
                sys.stderr.write("modified tree: %s\n" % (a_tree))
        ...
        # FIXME: currently setting the token2word hash and word2token hash to null as the tree has chanced
        a_tree.token2word_hash = {}
        a_tree.word2token_hash = {}
...
class tree_document:
    ...
    def align_to(self, another_tree_document, map_differences=True, alignment_mode="auto",
                 junk_character=None, junk_leaf=None, smart_map_traces=True,
                 trace_statistics_callback=None):
        """

        given another tree document, return a hash to lists of
        leaves in that tree document from leaves in this tree
        document:

        .. code-block:: python

            { leaf(pro-wal-mart) : [ leaf(pro), leaf(-), leaf(wal-mart)], ... }

        or in the other direction:

        .. code-block:: python

            { leaf(pro)      : [ leaf(pro-wal-mart) ],
              leaf(-)        : [ leaf(pro-wal-mart) ],
              leaf(wal-mart) : [ leaf(pro-wal-mart) ] }

        Traces are mapped too.  If 'smart_map_traces' is False,
        ...
        Can deal with some misalignments, but is not sentient.

        If map_differences is true, a change from ' e ' to ' a ', for
        example, will be included in the mapping.  Otherwise, it's
        dropped.

        Alignment mode may be either 'character' or 'token'.  If the
        ...
        is one exception: if the documents are *exactly* the same with
        ...
        Set junk_X to functions of one argument returning a boolean.
        If they are set, they will be called for each character / leaf
        and alignment will only be done with the ones for which they
        return negative.

        Set trace_statistics_callback to a function of three arguments
        if you want to get stats.  For example:

          trace_statistics_callback('all', 'totals', 'gold')
          trace_statistics_callback('argument', 'totals', 'gold')

        If unset the function is ignored.  The first argument of the
        call with be 'all' for data on all traces, 'argument' for data
        on argument bearing traces only, or 'error' for alignment
        errors.

        For non-errors, the possible second and third arguments are called for:


          'totals', 'gold': each trace in the from tree
          ...
          'secondary', various other stats

        """

        a, b = bunch(tree_document=self), bunch(tree_document=another_tree_document)

        if not junk_character:
            junk_character = lambda x: False

        if not junk_leaf:
            junk_leaf = lambda x: False

        def find_remaining_gaps():
            return on.common.util.find_gaps(
                leaf_in_us_2_leaf_in_them,
                [a_leaf for a_tree in a.tree_document for a_leaf in a_tree],
                [b_leaf for b_tree in b.tree_document for b_leaf in b_tree])

        def extend_mapping_with_trace_maps():
            """ this does another round of diff align, this time just on traces """

            for gap_a, gap_b in find_remaining_gaps():
                ga, gb = bunch(gap=gap_a), bunch(gap=gap_b)

                for gv in [ga, gb]:
                    gv.seq_to_leaf = []
                    gv.seq = []
                    for gv_leaf in gv.gap:
                        if gv_leaf.is_trace():
                ...
                if ga.seq and gb.seq:
                    for ga_seq_idx, gb_seq_idx in on.common.util.diff_align(ga.seq, gb.seq, map_differences=True).items():
                        ...
                        if gb_leaf not in leaf_in_us_2_leaf_in_them[ga_leaf]:
                            leaf_in_us_2_leaf_in_them[ga_leaf].append(gb_leaf)
                        if ga_leaf not in leaf_in_them_2_leaf_in_us[gb_leaf]:
                            leaf_in_them_2_leaf_in_us[gb_leaf].append(ga_leaf)

        def calculate_trace_statistics():
            insertions = [] # [b_trace_leaf, ...]
            deletions = []  # [a_trace_leaf, ...]
            matches = []    # [(a_trace_leaf, b_trace_leaf), ...]
            # matches may still differ in aspects other than position

            def is_prop_tagged(x):
                while x.parent:
                    if x.argument_node_list or x.link_node_list:
                        return True
                    if len(x.parent.children) != 1:
                        return False
                    x = x.parent
                return False

            def lowest_version(x):
                while not x.is_leaf() and len(x.children) == 1:
                    x = x.children[0]
                return x

            def follow_back(x):
                """ follow along a chain until we can't any more """
                while x.identity_subtree:
                    x = lowest_version(x.identity_subtree)
                return x

            def punct(a_leaf):
                return all(not x.isalnum() for x in a_leaf.word)

            def remove_edge_punct(leaf_list):
                while leaf_list and punct(leaf_list[0]):
                    leaf_list.pop(0)
                while leaf_list and punct(leaf_list[-1]):
                    leaf_list.pop(-1)
                return leaf_list

            def agree_in_target(x_leaf, y_leaf):
                if x_leaf.identity_subtree and y_leaf.identity_subtree:
                    x_leaves = remove_edge_punct(follow_back(x_leaf).leaves())
                    y_leaves = remove_edge_punct(follow_back(y_leaf).leaves())

                    x_words = [x.get_word() for x in x_leaves if not x.is_trace()]
                    y_words = [y.get_word() for y in y_leaves if not y.is_trace()]

                    return x_words == y_words
                return not x_leaf.identity_subtree and not y_leaf.identity_subtree

            def agree_in_indexing(x_leaf, y_leaf):
                return bool(not x_leaf.identity_subtree and not y_leaf.identity_subtree) or \
                       bool(x_leaf.identity_subtree and y_leaf.identity_subtree)

            note = trace_statistics_callback
            def notefor(a_leaf, type, thenote):
                note('all', type, thenote)
                if is_prop_tagged(a_leaf):
                    note('argument', type, thenote)

            def appendthem(missing, found, td, hash, usename):
                """ set found to none if we don't care about matches """
                for x_tree in td:
                    for x_leaf in x_tree:
                        if x_leaf.is_trace():

                            notefor(x_leaf, "totals", usename)

                            if usename == "gold":
                                notefor(x_leaf, "secondary", "total_traces")
                                if x_leaf.identity_subtree:
                                    notefor(x_leaf, "secondary", "total_indexed_traces")
                                else:
                                    notefor(x_leaf, "secondary", "total_unindexed_traces")

                            if x_leaf in hash:
                                if found is not None:
                                    y_leaves = hash[x_leaf]
                                    if len(y_leaves) != 1:
                                        note('error', '%s trace maps to multiple leaves' % usename)
                                    else:
                                        y_leaf = y_leaves[0]
                                        if not y_leaf.is_trace():
                                            note('error', '%s trace maps to non-trace' % usename)
                                        else:
                                            found.append((x_leaf, y_leaf))
                            else:
                                missing.append(x_leaf)

            appendthem(insertions, None, b.tree_document, leaf_in_them_2_leaf_in_us, "auto")
            appendthem(deletions, matches, a.tree_document, leaf_in_us_2_leaf_in_them, "gold")

            for a_leaf in deletions:
                notefor(a_leaf, "primary", 'deleted')

                notefor(a_leaf, "secondary", 'total_deleted_traces')
                if a_leaf.identity_subtree:
                    notefor(a_leaf, "secondary", 'deleted_indexed_traces')
                else:
                    notefor(a_leaf, "secondary", 'deleted_unindexed_traces')

            for b_leaf in insertions:
                note('all', "primary", 'inserted') # we can't tell if it should have been seen as argument bearing
                note('all', "secondary", 'total_inserted_traces')
                if b_leaf.identity_subtree:
                    note('all', "secondary", 'inserted_indexed_traces')
                else:
                    note('all', "secondary", 'inserted_unindexed_traces')

            for a_leaf, b_leaf in matches:
                same_type = a_leaf.trace_type == b_leaf.trace_type
                same_indexing = agree_in_indexing(a_leaf, b_leaf)
                same_target = agree_in_target(a_leaf, b_leaf)


                if same_type and same_indexing and same_target:
                    notefor(a_leaf, "primary", 'correct')
                else:
                    to_note = ["incorrect"]
                    if not same_type:
                        to_note.append("type")

                    if not same_indexing:
                        to_note.append("indexing")
                    elif not same_target: # we only want to check if they share a target if they share indexing
                        to_note.append("target")
                    notefor(a_leaf, "primary", ' '.join(to_note))


                if same_indexing and same_target and same_type:
                    notefor(a_leaf, "secondary", "total_correct_traces")
                    if a_leaf.identity_subtree:
                        notefor(a_leaf, "secondary", "correctly_indexed_traces")
                    else:
                        notefor(a_leaf, "secondary", "correctly_unindexed_traces")
                else:
                    if a_leaf.identity_subtree and b_leaf.identity_subtree and not same_target:
                        notefor(a_leaf, "secondary", "incorrectly_indexed_traces")
                    else:
                        notefor(a_leaf, "secondary", "total_substituted_traces")
                        if a_leaf.identity_subtree and not b_leaf.identity_subtree:
                            notefor(a_leaf, "secondary", "substituted_unindexed_for_indexed")
                        elif not a_leaf.identity_subtree and b_leaf.identity_subtree:
                            notefor(a_leaf, "secondary", "substituted_indexed_for_unindexed")
                        elif not same_type:
                            if a_leaf.identity_subtree:
                                notefor(a_leaf, "secondary", "substituted_type_for_other_type_indexed")
                            else:
                                notefor(a_leaf, "secondary", "substituted_type_for_other_type_unindexed")
                        else:
                            assert False, (same_indexing, same_target, same_type)




        def exactly_the_same_ignoring_traces(tda, tdb):
            def make_sig(tdx):
                return [[wordfor(x_leaf) for x_leaf in x_tree if not x_leaf.is_trace()]
                        for x_tree in tdx]
            return make_sig(tda) == make_sig(tdb)

        def wordfor(a_leaf):
            word = "".join([c for c in a_leaf.get_word() if not junk_character(c)])

            if a_leaf.get_root().language.startswith("ar"):
                word = on.common.util.cannonical_arabic_word(word)
            return word

        if alignment_mode == "auto":
            if exactly_the_same_ignoring_traces(self, another_tree_document):
        ...
        def add_to_seq(v, leaf, alignment_item):
            v.seq_to_leaf.append(leaf)
            v.seq.append(alignment_item)

        for v in [a,b]:
            v.seq_to_leaf = [] # mapping back from the alignment sequence to leaves in v
            v.seq = []
            for a_tree in v.tree_document:

                for a_leaf in a_tree.leaves():

                    if not junk_leaf(a_leaf):
                        word = wordfor(a_leaf)

                        if not word:
                            continue

                        if smart_map_traces and a_leaf.is_trace():
                            continue

                        if alignment_mode == "token":
                            add_to_seq(v, a_leaf, word)
                        elif alignment_mode == "character":
                            for c in word:
                                add_to_seq(v, a_leaf, c)
                        else:
                            raise Exception("Invalid alignment_mode: %s; should be 'token' or 'character'" % alignment_mode)
        ...
        for a_seq_idx, b_seq_idx in on.common.util.diff_align(a.seq, b.seq, map_differences).items():
            ...
            if b_leaf not in leaf_in_us_2_leaf_in_them[a_leaf]:
                leaf_in_us_2_leaf_in_them[a_leaf].append(b_leaf)
            if a_leaf not in leaf_in_them_2_leaf_in_us[b_leaf]:
                leaf_in_them_2_leaf_in_us[b_leaf].append(a_leaf)

        if smart_map_traces:
            extend_mapping_with_trace_maps()

        if trace_statistics_callback:
            calculate_trace_statistics()

        return leaf_in_us_2_leaf_in_them
    ...
    def dump_view(self, a_cursor=None, out_dir="", buckwalter=True, vocalized=False):
        # write view file
...
whether there are more than one parses per timing range, and correct accordingly:
    ...
    def copy_to_different_trees(self, alignments, to_treebank, from_treebank):
        """ this doesn't make sense for treebanks -- see copy_banks_from """
        raise Exception("treebank.copy_to_different_trees not supported")

    def copy_banks_from(self, from_treebank, banks=None, map_differences=True, trace_statistics_callback=None,
                        ignore_errors=False, alignment_mode="auto"):
        ...
        assert "parallel" not in banks or len(self) == len(from_treebank)

        for from_tree_document in from_treebank:
            if from_tree_document not in self:
                continue
            to_tree_document = self.get_document(from_tree_document)

            def junk_leaf(a_leaf):

                if a_leaf.id.endswith("@ch@tc@en@on"):
                    """
                    for english callhome we want to be sure not to
                    align any leaves that are not legitamate tokens.
                    Those are the E_S and N_S metadata, the
                    CODE-speaker metadata, and anything wrapped in
                    braces.
                    """
                    ...
                    for brace_start, brace_end in ["{}","<>","[]"]:
                        if w.startswith(brace_start) and w.endswith(brace_end):
                            return True

                    if a_leaf.tag == "CODE" and w.count("_") == 2 and w.count(".") == 2:
                        return True


                if a_leaf.id.endswith("@bc@en@on") or a_leaf.id.endswith("@bn@en@on"):

                    parent = a_leaf.parent
                    while( parent != None):

                        if(parent.phrase_type == "EDITED"):
                            #print "leaf:", a_leaf.start, a_leaf.end, a_leaf.id
                            #print "parent:", parent
                            return True
                    ...
                    #leaves = p.leaves()
                    #
                    #if( a_leaf.start+1 < len(leaves) and leaves[a_leaf.start+1].word == a_leaf.word ):
                    #    print "found repeat words", leaves[a_leaf.start:]
                    #
                    #    return True
            ...
            from_to = from_tree_document.align_to(to_tree_document, map_differences,
                                                  trace_statistics_callback=trace_statistics_callback,
                                                  alignment_mode=alignment_mode,junk_leaf=junk_leaf)
            ...
            with codecs.open("alignments/%s--%s.txt"%(from_tree_document.document_id.split("/")[-1], from_treebank.tag), "w", "utf8") as outf:
                ...
                for a_leaf, b_leaf_list in from_to.items():
                    ...
                    b_ti = -1
                    if b_leaf_list:
                        b_si = b_leaf_list[0].get_sentence_index()
                        b_ti = b_leaf_list[0].get_token_index()

                    outf.write("  %s %s -> %s %s :: %s -> %s\n" % (
                        ("%s"%a_leaf.get_sentence_index()).zfill(5),
                        ("%s"%a_leaf.get_token_index()).zfill(5),
                        ("%s"%b_si).zfill(5),
                        ("%s"%b_ti).zfill(5),
                        a_leaf.get_word(),
                        ", ".join(b_leaf.get_word() for b_leaf in b_leaf_list)))
                    for b_leaf in b_leaf_list:
                        if a_leaf not in to_from[b_leaf]:
                            to_from[b_leaf].append(a_leaf)

                    def simplify(a_word):
                        if a_leaf.get_root().language.startswith("ar"):
                            return on.common.util.cannonical_arabic_word(a_word)
                        return a_word

                    if not b_leaf.is_trace():
                        """ don't count traces either way """
                        simple_word = simplify(a_leaf.get_word())
                        expected_simple_word = simplify("".join(b_leaf.get_word() for b_leaf in b_leaf_list))


                        if simple_word == expected_simple_word:
                            alignments += 1

                        should_have_aligned += 1

                on.common.log.report("copy_banks_from", "alignment fraction", document_id=to_tree_document.document_id,
                                     valid_alignments=alignments, total_alignments=len(from_to), total_tokens=should_have_aligned,
                                     fraction=alignments * 1.0 / should_have_aligned, from_tag=from_treebank.tag, to_tag=self.tag)

                if alignments * 1.0 / should_have_aligned < .5:
                    on.common.log.report("copy_banks_from", "alignment fraction under .5  very SERIOUS", document_id=to_tree_document.document_id,
                                         valid_alignments=alignments, total_alignments=len(from_to), total_tokens=should_have_aligned,
                                         from_tag=from_treebank.tag, to_tag=self.tag)
                elif alignments * 1.0 / should_have_aligned < .85:
                    on.common.log.report("copy_banks_from", "alignment fraction under .85  SERIOUS", document_id=to_tree_document.document_id,
                                         valid_alignments=alignments, total_alignments=len(from_to), total_tokens=should_have_aligned,
                                         from_tag=from_treebank.tag, to_tag=self.tag)



            alignments_from_to[from_tree_document.document_id] = from_to
            alignments_to_from[from_tree_document.document_id] = to_from




        for bank in banks:
            if bank not in from_treebank.banks:
                on.common.log.status("Not copying bank %s from treebank %s to treebank %s because the other treebank doesn't have that bank." % (
                    bank, from_treebank.id, self.id))
                continue

            from_bank = from_treebank.banks[bank]

            existant_to_bank = None
            if bank in self.banks:
                existant_to_bank = self.banks[bank]

            on.common.log.status("bank=%s, nonexistent=%s" % (bank, existant_to_bank == None))


            to_bank = from_bank.copy_to_different_trees(alignments_from_to, alignments_to_from,
                                                        to_treebank=self, from_treebank=from_treebank,
                                                        existant_to_bank=existant_to_bank,
                                                        ignore_errors=ignore_errors) # includes enrichment

            if existant_to_bank is None:
                self.subcorpus.add_bank(to_bank.extension, to_bank)
            else:
                assert to_bank is existant_to_bank
...
"""
    ...
    @staticmethod
    def build_root_to_root(alignment_hash):
        """ turn a leaf to leaf mapping like you get from tree_document.align_to into a root to root mapping """

        r_2_r = defaultdict(set)

        for l_a in alignment_hash:
            for l_b in alignment_hash[l_a]:
                r_2_r[l_a.get_root()].add(l_b.get_root())

        return r_2_r


    @staticmethod
    def smart_unmapper(alignment_from_to, from_leaf, beginning_or_end):
        """ return to_leaf, trying to recover if from_leaf is not in the mapping

        beginning_or_end should be 'beginning' if we should map as early
        as possible, or 'end' if we should map as late as possible

        Recovery is currently triggered only in cases where the leaf in
        question is a trace.

        """

        if beginning_or_end not in ["beginning", "end"]:
            raise Exception("beginning_or_end should be either 'beginning' or 'end'")

        sentence, word = int(from_leaf.get_sentence_index()), int(from_leaf.get_token_index())

        def unmap(from_leaf):
            which_leaf = 0 # beginning
            if beginning_or_end == "end":
                which_leaf = -1

            if from_leaf not in alignment_from_to:
                return None
        ...
        for w in look_range:
            ...
            up. But if it is a trace, we'll try previous leaves and see if
            they have mappings."""

            from_leaf = from_leaf.get_root().get_leaf_by_token_index(w)
            u = unmap(from_leaf)

            if u:
                return u
            else:
                if not from_leaf.is_trace():
                    return None # only try the next leaf if we're a trace

        return None # even trying other leaves we couldn't find a match
