...
class coreference_link(on.corpora.tree_alignable_sgml_span):
    ...
    def copy_to_different_trees(self, alignment_from_to, our_coref_doc, other_coref_doc, other_coref_chain):
        ...
        if not self.start_leaf or not self.end_leaf:
            return None
            #raise Exception("coreference_link.copy_to_different_trees requires enrichment to have happened already")

        a_coref_link = self.clone_to(on.corpora.tree.treebank.smart_unmapper(alignment_from_to, self.start_leaf, "beginning"),
                                     on.corpora.tree.treebank.smart_unmapper(alignment_from_to, self.end_leaf, "end"),
                                     other_coref_chain)

        if (not a_coref_link.start_leaf or not a_coref_link.end_leaf or
            a_coref_link.start_leaf.get_token_index() > a_coref_link.end_leaf.get_token_index()):

            on.common.log.report("copy_to_different_trees",
                                 "failed to map coreference link",
                                 id=self.coreference_chain_id,
                                 self_id=self.id,
                                 old_tree_no=self.start_leaf.get_root().get_sentence_index(),
                                 start_leaf=a_coref_link.start_leaf,
                                 end_leaf=a_coref_link.end_leaf,
                                 string=self.string,
                                 old_tree=self.start_leaf.get_root().pretty_print())

            a_coref_link.start_leaf = None
            a_coref_link.end_leaf = None

            return None

        if a_coref_link.start_leaf.get_sentence_index() != a_coref_link.end_leaf.get_sentence_index():
            span = self.string
            old_tree = self.start_leaf.get_root()
            new_tree_A = a_coref_link.start_leaf.get_root()
            new_tree_B = a_coref_link.end_leaf.get_root()
            lang = old_tree.language
            if lang == "ar":
                span = on.common.util.buckwalter2unicode(span)

            if our_coref_doc.adjudicated:
                on.common.log.bad_data_long(lang, "coref", "coref chain spans sentence boundry in new trees",
                                            ["span", span],
                                            ["id", a_coref_link.coreference_chain.identifier],
                                            ["document id",  a_coref_link.coreference_chain.document_id],
                                            ["old tree", old_tree.get_word_string()],
                                            #["first new tree index", new_tree_A.get_sentence_index()],
                                            ["first new tree", new_tree_A.get_word_string()],
                                            #["second new tree index", new_tree_B.get_sentence_index()],
                                            ["second new tree", new_tree_B.get_word_string()])
            return None

        if not a_coref_link.subtree:
            # if we're not aligned but could be by changing how we view
            # traces, pos, det, punct etc, then change our start and end
            # to match
            new_start, new_end = a_coref_link.start_leaf.get_root().get_closest_aligning_span(a_coref_link.start_leaf, a_coref_link.end_leaf)
            if not new_start or not new_end:
                # lets be even laxer
                new_start, new_end = a_coref_link.start_leaf.get_root().get_closest_aligning_span(a_coref_link.start_leaf, a_coref_link.end_leaf, strict_alignment=False)

            if new_start and new_end:
                a_coref_link.start_leaf, a_coref_link.end_leaf = new_start, new_end


        return a_coref_link

    def clone_to(self, new_start_leaf, new_end_leaf, new_coref_chain):
        return self._clone_to(new_start_leaf, new_end_leaf,
                              coreference_link(self.type, new_coref_chain, a_cursor = None, precise=self.precise))
...
class coreference_chain(object):
    ...
    def copy_to_different_trees(self, us_to_different, our_coref_doc, other_coref_doc):
        a_coref_chain = coreference_chain(self.type, self.identifier,
                                          self.section, self.document_id,
                                          a_cursor=None, speaker=self.speaker)

        a_coref_chain.coreference_links = [a_link.copy_to_different_trees(us_to_different, our_coref_doc, other_coref_doc, a_coref_chain)
                                           for a_link in self if a_link.valid]
        a_coref_chain.coreference_links = [x for x in a_coref_chain if x]

        return a_coref_chain
...
class coreference_document:
    ...
    def filter_overlapping_sub_links(self):
        ...
        for coreference_chain_id in list(self.coreference_chain_hash.keys()):
            ...
            if len(coref_chain) > 1:
                on.common.log.debug("found a chain of length > 1: id: %s\n" % coreference_chain_id.split("@")[1], on.common.log.DEBUG, on.common.log.MIN_VERBOSITY)
                    ...
                    for i in range(0, len(coref_chain)):
                        for j in range(i+1, len(coref_chain)):
                            ...
                            if( coref_chain[i].overlaps(coref_chain[j]) and (coref_chain[i].primary_start_index == coref_chain[j].primary_start_index)):
                                ...
                                OVERLAP = True
                                on.common.log.debug("found a overlapping pair in a > 1 link chain.  deleting the shorter link\n", on.common.log.DEBUG, on.common.log.MIN_VERBOSITY)

                                a = int(coref_chain[i].primary_end_index)
                                b = int(coref_chain[i].primary_start_index)
                                c = int(coref_chain[j].primary_end_index)
                                d = int(coref_chain[j].primary_start_index)

                                del coref_chain[i if (a-b) < (c-d) else j]
                                break
    ...
    def copy_to_different_trees(self, alignment_from_to, alignment_to_from,
                                from_tree_document, to_tree_document, to_bank,
                                existing_to_document=None):
        """ given a leaf to leaf mapping from the trees we were
        annotated against to a new set of trees, return a new
        coreference document that's as if it had been annotated
        against the new set of trees.

        Big Warning: if you want ids to continue to have their proper
        semantic meaning (where the stuff between the '@'s corresponds
        to the real world), you need to dump and reload the
        'different' coref bank after running this.  After a dump and
        reload ids should be consistent again)
        """

        assert existing_to_document is None

        def get_from_tree(to_tree):
            votes=defaultdict(int) # tree -> votes for that tree

            for to_leaf in to_tree.leaves():
                if to_leaf in alignment_to_from:
                    for from_leaf in alignment_to_from[to_leaf]:
                        votes[from_leaf.get_root()] += 1

            best_tree = None
            for a_tree in votes:
                if not best_tree or votes[a_tree] > votes[best_tree]:
                    best_tree = a_tree

            return best_tree # None if we didn't find one

        def copy_section_names():
            coref_section = from_tree_document[0].coref_section
            for to_tree in to_tree_document:
                from_tree = get_from_tree(to_tree)
                if from_tree:
                    coref_section = from_tree.coref_section
                to_tree.coref_section = coref_section

        to_coref_doc = coreference_document("", self.document_id, to_bank.extension,
                                            a_cursor="this is to say don't load from files")
...
class coreference_bank(abstract_bank):
    ...
    def __init__(self, a_subcorpus, tag, a_cursor=None, extension="coref", indexing="token"):
                        ...
                        for b_coreference_link in a_leaf.start_coreference_link_list:
                            if (a_coreference_link not in reported_links and
                                ...
                                if a_coreference_link.coreference_chain is not b_coreference_link.coreference_chain:
                                    ...
                                    if a_coreference_document.adjudicated:
                                        on.common.log.bad_data_long(
                                            a_tree.language, "coref",
                                            "two conflicting coref chains include the same leaf span " +  has_hyphen,
                                            ["document id", a_tree_document.document_id],
                                            ["part", a_tree.coref_section],
                                            ["span", span],
                                            ["sentence", sentence],
                                            ["chain id A", a_coreference_link.coreference_chain.identifier],
                                            ["chain id B", b_coreference_link.coreference_chain.identifier],
                                            ["links in chain A", chain_links_A],
                                            ["links in chain B", chain_links_B])
                                else:
