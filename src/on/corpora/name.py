...
class name_entity(on.corpora.tree_alignable_sgml_span):
    ...
    def clone_to(self, a_start_leaf, a_end_leaf):
        return self._clone_to(a_start_leaf, a_end_leaf, name_entity(None, self.document_id, self.type, None, None, None))
    ...
    def copy_to_different_trees(self, alignment_from_to):
        ...
        if not self.start_leaf or not self.end_leaf:
            raise Exception("name_entity.copy_to_different_trees requires enrichment to have happened already")


        to_name_entity = self.clone_to(on.corpora.tree.treebank.smart_unmapper(alignment_from_to, self.start_leaf, "beginning"),
                                       on.corpora.tree.treebank.smart_unmapper(alignment_from_to, self.end_leaf, "end"))

        if (not to_name_entity.start_leaf or not to_name_entity.end_leaf or
            to_name_entity.start_leaf.get_sentence_index() is None or
            to_name_entity.start_leaf.get_sentence_index() > to_name_entity.end_leaf.get_sentence_index()):

            on.common.log.report("copy_to_different_trees", "name mapping failed", id=self.id)
            return None

        elif to_name_entity.start_leaf.get_sentence_index() != to_name_entity.end_leaf.get_sentence_index():
            span = self.string
            old_tree = self.start_leaf.get_root()
            new_tree_A = to_name_entity.start_leaf.get_root()
            new_tree_B = to_name_entity.end_leaf.get_root()
            lang = old_tree.language
            if lang == "ar":
                span = on.common.util.buckwalter2unicode(span)

            on.common.log.bad_data_long(lang, "name", "named entity spans sentence boundry in new trees",
                                        ["span", span],
        ...
        # to match
        if not to_name_entity.subtree:
            new_tree = to_name_entity.start_leaf.get_root()
            new_start, new_end = new_tree.get_closest_aligning_span(to_name_entity.start_leaf, to_name_entity.end_leaf)

            if not new_start or not new_end:
                # lets be even laxer
                new_start, new_end = new_tree.get_closest_aligning_span(to_name_entity.start_leaf, to_name_entity.end_leaf, strict_alignment=False)

            if new_start and new_end:
                to_name_entity.start_leaf, to_name_entity.end_leaf = new_start, new_end

        return to_name_entity
        ...
        for a_name_entity_list in self.name_entity_hash.values():
            for a_name_entity in a_name_entity_list:
                if a_name_entity.valid:
                    ...
                    if new_name_entity: # copy_to_different_trees returns None on failure
                        new_name_entity.check_tree_alignment()
                        to_name_entity_set.append(new_name_entity)
                else:
                    on.common.log.report("name", "dropping invalid NE")

        return to_name_entity_set
...
class name_tagged_document:
   ...
   %s
        ...
        """ % ("-"*80, self.document_id, sentence, "-"*80), on.common.log.DEBUG, on.common.log.MAX_VERBOSITY)
                ...
                for token_index, token in enumerate(tokens):
                    ...
                    if(len(name_tagged_document.end_ne_re.findall(token)) > 0):
                        ...
                        if(add_to_db_flag == True or on.common.global_flags.ALL_NAMES == True):

                            #---- add it only if it is a fac and NO_FAC is not true ----#
                            if(is_fac == True and on.common.global_flags.NO_FAC == True):
                                pass
                            else:
    ...
    def copy_to_different_trees(self, alignment_from_to, alignment_to_from,
        ...
        a_name_doc = name_tagged_document("", to_tree_document.document_id,
                                          to_bank.extension, a_cursor="don't load from files")

        for a_name_entity_set in self.name_entity_sets:
            a_name_doc.name_entity_sets.append(a_name_entity_set.copy_to_different_trees(alignment_from_to))
