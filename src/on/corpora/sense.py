...
class on_sense(object):
    ...
    def copy_to_different_trees(self, alignment_from_to):
        ...
        def unmap_fail(errcode):
            on.common.log.reject(["docid", self.document_id, "sense"], "sense",
                                 [[errcode, ["from_leaf: %s" % self.leaf,
                                             "on_sense: %s" % self,
                                             "leaf present: %s" % (self.leaf in alignment_from_to),
                                             "to_leaves: %s" % alignment_from_to[self.leaf] if self.leaf in alignment_from_to else []
                                             ]]],
                                 self.enc_sense)
            return None

        if not self.leaf in alignment_from_to:
            return unmap_fail("unaligned")
        ...
        candidate_leaves = [to_leaf for to_leaf in alignment_from_to[self.leaf] \
                            if nvpos(to_leaf) == nvpos(self.leaf)]

        to_leaf = None
        for a_leaf in candidate_leaves:
            """ take the candidate leaf with the longest word """
            if not to_leaf or len(a_leaf.word) > len(to_leaf.word):
                to_leaf = a_leaf

        if not to_leaf:
            return unmap_fail("posmm")

        return self.clone_to(to_leaf)
    ...
    def clone_to(self, a_leaf):
        """ make a duplicate of this sense annotation and put it on a_leaf """

        a_on_sense = on_sense(document_id=None,
                              tree_index=None,
                              word_index=None,
                              lemma=            self.lemma,
                              pos=              self.pos,
                              ann_1_sense=      self.ann_1_sense,
                              ann_2_sense=      self.ann_2_sense,
                              adj_sense=        self.adj_sense,
                              sense=            self.sense,
                              adjudicated_flag= self.adjudicated_flag)

        a_on_sense.leaf = a_leaf
        a_on_sense.enc_sense = self.enc_sense # setting for reference, though likely not accurate
        return a_on_sense
...
class on_sense_lemma_type(on.corpora.abstract_open_type_table):
    ...
    Inter-annotator-agreement (ita) is tricky for :class:`on_sense`
    ...
    Statistics:
     ...
     - ``count`` -- the number of times we had individual annotator information for this sense
     - ``ann_1_2_agreement`` -- the percentage of the time that annotator 1 and annotator 2 agreed on the sense


    Because adjudication lags behind annotation and we do not include
    sense annotations that are unadjudicated disagreements, the
    ``ann_1_2_agreement`` will be slightly higher than it ought to be
    for considering how difficult a lemma is to tag.  Consider the
    following sense annotations:
    ...
    The first three examples have been adjudicated, the final four
    have not.  One would expect, then, for ``count`` to be 7 and
    ``ann_1_2_agreement`` 57 percent.  But the two unadjudicated
    disagreements will be reserved for adjudication, so we're really
    computing these statistics on:
    ...
    This decreases ``count`` to 5 while keeping the number of times
    ann_1 and ann_2 agreed at 4, raising ``ann_1_2_agreement`` to 80
    ...
    is something to be aware of for now.
...
class sense_tagged_document:
    ...
    def __init__(self, sense_tagged_document_string, document_id,
        ...
        if(a_cursor == None):
            ...
            for enc_sense in sense_tagged_document_string.split("\n"):
                ...
                if(lang != "ch"):
                    if(document_id.split("@")[0] != self.document_id.split("@")[0]):
    ...
    def copy_to_different_trees(self, alignment_from_to, alignment_to_from,
        ...
        if existing_to_document is None:
            to_sense_document = sense_tagged_document(sense_tagged_document_string="",
                                                      document_id=self.document_id,
                                                      a_sense_bank=to_bank,
                                                      a_cursor="don't read any files",
                                                      preserve_ita=True)
        else:
            to_sense_document = existing_to_document

        for from_on_sense in self.on_sense_list:
            if not from_on_sense.valid:
                continue

            to_on_sense = from_on_sense.copy_to_different_trees(alignment_from_to)
            if to_on_sense:
                to_sense_document.on_sense_list.append(to_on_sense)
                to_sense_document.lemma_pos_hash["%s-%s" % (to_on_sense.lemma, to_on_sense.pos)] = 0

                genre, source, section, fname = self.document_id.split("@")[0].split("/")
                new_path = "%s_%s/%s/%s.%s_word" % (genre, source, section, fname, to_tree_document.tag)

                from_tree_str = from_on_sense.leaf.get_root().pretty_print()
                to_tree_str = to_on_sense.leaf.get_root().pretty_print()

                identical = from_tree_str == to_tree_str

                on.common.log.adjust(["docid", self.document_id, "sense"],
                                     #"sense" if not identical else "universal",
                                     #[["copyok" if not identical else "ok", [] ]],
                                     "universal",
                                     [["ok", [] ]],
                                     to_on_sense.enc_sense,
                                     "%s %s %s %s" % ( new_path,
                                                       to_on_sense.tree_index, to_on_sense.word_index,
                                                       to_on_sense.sense_annotation(preserve_ita=True)))

        return to_sense_document
