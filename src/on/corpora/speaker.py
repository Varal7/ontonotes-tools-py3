...
"""
    ...
    def copy_to(self, a_tree):
        to_speaker_sentence = speaker_sentence(self.line_number, self.document_id, self.start_time, self.stop_time,
                                               self.name, self.gender, self.competence)
        to_speaker_sentence.enrich_tree(a_tree)
        return to_speaker_sentence
...
class speaker_document:
    ...
    def copy_to_different_trees(self, alignment_from_to, alignment_to_from,
        ...
        root_level_to_from = on.corpora.tree.treebank.build_root_to_root(alignment_to_from)

        for to_root in root_level_to_from:
            if len(root_level_to_from[to_root]) > 1:
                raise Exception("speaker_sentence.copy_to_different_trees does not support alignments other " +
                                "than addition or deletion of trees.  Got trees {%s} for \n\n%s" % (
                    "\n\n\n".join([a.pretty_print() for a in root_level_to_from[to_root]]), to_root.pretty_print()))

        to_speaker_document = speaker_document(self.document_id, to_bank.extension)

        for to_tree in to_tree_document:
            if to_tree in root_level_to_from:
                from_tree = root_level_to_from[to_tree].pop()
                if from_tree.speaker_sentence:
                    to_speaker_sentence = from_tree.speaker_sentence.copy_to(to_tree)
                    to_speaker_document.append(to_speaker_sentence)

        return to_speaker_document
