...
class abstract_bank(object):
    ...
    .. code-block:: python
       ...
       if load_from_files:
          a_something_bank = something_bank(a_subcorpus, tag[, optional arguments])
       else: # load from db
          a_something_bank = something_bank.from_db(a_subcorpus, tag, a_cursor[, optional arguments])

       # only if a_something_bank is not a treebank
       a_something_bank.enrich_treebank(a_treebank[, optional_arguments])

       for a_something_document in a_something_bank:
          # get the corresponding somethingelse document
          a_somethingelse_document = a_somethingelse_bank.get_document(a_something_document)

       if write_to_files:
          a_something_document.dump_view(a_cursor=None, out_dir)
       else: # write to db
          a_something_document.write_to_db(a_cursor)


       if transfer_annotations_to_new_trees:
          a_old_treebank = get_treebank()
          a_old_sense_bank = get_sense_bank()
          a_old_sense_bank.enrich_treebank(a_old_treebank)

          a_new_treebank = get_treebank()

          a_new_sense_bank = a_old_sense_bank.copy_to_different_trees(alignments, a_new_treebank, a_old_treebank)
    ...
    def copy_to_different_trees(self, alignments_from_to, alignments_to_from,
                                to_treebank, from_treebank, existant_to_bank=None,
                                ignore_errors=False):
        """
        alignments -- our tree document id -> leaf in our td to leaf in their td
        """

        if existant_to_bank is None:
            to_bank = self.__class__(from_treebank.subcorpus, to_treebank.tag,
                                     a_cursor="please don't load from files")
        else:
            to_bank = existant_to_bank

        status("Copying %s:%s to %s:%s ..." % (self.extension, from_treebank.extension, to_bank.extension, to_treebank.extension))

        for from_document in self:
            if not hasattr(from_document, "copy_to_different_trees"):
                continue

            from_tree_document = from_treebank.get_document(from_document)
            to_tree_document = to_treebank.get_document(from_document)

            alignment_from_to = alignments_from_to[from_tree_document.document_id]
            alignment_to_from = alignments_to_from[from_tree_document.document_id]

            existing_to_document = None
            if to_tree_document in to_bank:
                existing_to_document = to_bank.get_document(to_tree_document)


            kw_args = {"alignment_from_to": alignment_from_to,
                       "alignment_to_from": alignment_to_from,
                       "from_tree_document": from_tree_document,
                       "to_tree_document": to_tree_document,
                       "to_bank": to_bank,
                       "existing_to_document": existing_to_document}

            if ignore_errors and self.extension.endswith("prop"):
                kw_args["ignore_errors"] = ignore_errors

            to_document = from_document.copy_to_different_trees(**kw_args)

            if existing_to_document is None:
                to_document.tree_document = to_tree_document
                to_bank.append(to_document)
            else:
                assert to_document is existing_to_document

        if existant_to_bank is None:
            to_treebank.inform_enriched(to_bank)
...
class subcorpus(UserDict.DictMixin):
    ...
    def __getitem__(self, key):
        ...
        .. code-block:: python
          ...
          for a_tree_document, a_sense_document in zip(a_subcorpus['parse'], a_subcorpus['sense']):
             pass
    ...
    def get_unmapper(self):
        """ a hash from ontonotes names back to original document names

        For each language, in each genre directory, there is a file
        ``map.txt``.  For example:

        .. code-block:: bash

          $ head data/english/annotations/bn/map.txt
        ...
        These files map ontonotes names relative to the parent
        directory of ``map.txt`` (``bn``, in this case) back to the
        original document names.

        This function finds the appropriate map.txt for this subcorpus
        and parses it into a hash.

        This only works if we read the subcorpus information from
        disk.  The contents of ``map.txt`` are not available in the
        database.

        Sometimes there is a third column in map.txt.  This column
        ...
        under context.  This function ignores the third column.

        """

        if self.backed_by() != "fs":
            raise Exception("get_unmapper: only applicable to filesystem-backed subcorpora")

        unmapper = {}
        with codecs.open(os.path.join(self.base_dir, self.root_dir, "..", "map.txt"), "r", "utf8") as inf:
            for line in inf:
                m_onto, m_other = line.split()[:2]
                m_onto = m_onto.split(os.path.sep)[-1]
                unmapper[m_onto] = m_other
        return unmapper
...
class tree_alignable_sgml_span(object):
    ...
    def _clone_to(self, new_start_leaf, new_end_leaf, new_entity):

        new_entity.start_leaf = new_start_leaf
        new_entity.end_leaf = new_end_leaf

        if (not (new_start_leaf and new_end_leaf and new_start_leaf.get_root() == new_end_leaf.get_root()) or
            new_start_leaf.get_token_index() > new_end_leaf.get_token_index()):
            return new_entity

        if self.string == new_entity.string:
            return new_entity

        if same_except_for_tokenization_and_hyphenization(self.string, new_entity.string):
            #print "accepting differences between '%s' and '%s'" % (self.string, new_entity.string)
            return new_entity

        ### now we see if we can't use fewer new leaves
        new_tree = new_start_leaf.get_root()
        candidate_leaves = []
        for a_leaf in new_tree:
            if a_leaf is new_start_leaf or candidate_leaves:
                candidate_leaves.append(a_leaf)

            if a_leaf is new_end_leaf:
                break

        # now candidate leaves contains all possible start and end leaves
        start_end_candidates_ranked = []
        compare_with_old_string = SequenceMatcher()
        compare_with_old_string.set_seq2(self.string)

        for a_candidate_leaf in candidate_leaves:
            for b_candidate_leaf in candidate_leaves:
                if b_candidate_leaf.get_token_index() >= a_candidate_leaf.get_token_index():

                    new_entity.start_leaf = a_candidate_leaf
                    new_entity.end_leaf = b_candidate_leaf

                    if new_entity.start_leaf == a_candidate_leaf and new_entity.end_leaf == b_candidate_leaf:
                        # is not always true: if the leaves are traces
                        # then there's confuddling logic in
                        # _set_{start,end}_leaf that we want to bypass
                        # here.

                        compare_with_old_string.set_seq1(new_entity.string)
                        start_end_candidates_ranked.append([compare_with_old_string.ratio(), a_candidate_leaf, b_candidate_leaf])

        start_end_candidates_ranked.sort()

        #print "Copying '%s' to '%s'" % (self.string, ", ".join(x.word for x in candidate_leaves))
        #print "  Considering"
        #for ranking, start, end in start_end_candidates_ranked:
        #    print "    ", ranking, start, end

        ranking, new_entity.start_leaf, new_entity.end_leaf = start_end_candidates_ranked[-1]

        #print "  chose:", a_coref_link.string


        # save string_without_offsets for later
        new_entity.start_char_offset = 0
        new_entity.end_char_offset = 0
        string_without_offsets = new_entity.string

        # copy subtoken information.
        new_entity.start_char_offset = self.start_char_offset
        new_entity.end_char_offset = self.end_char_offset
        if same_except_for_tokenization_and_hyphenization(self.string, new_entity.string):
            """ if our string with offsets is the same as the
            new string with the same offsets, then keep them
            just as they are """
            pass
        else:
            def set_offsets():
                # a function so we can short circuit when we find the right place

                new_entity.start_char_offset = 0
                new_entity.end_char_offset = 0

                len_first_token = len(new_entity.string.split()[0])
                len_last_token = len(new_entity.string.split()[-1])


                # as we fiddle with a_coref_link.s_off and
                # a_coref_link.e_off this changes what
                # a_coref_link.string returns
                for new_entity.start_char_offset in range(len_first_token):
                    for new_entity.end_char_offset in range(len_last_token):
                        #print "  > comparing %s and %s" % (self.string, new_entity.string)
                        if same_except_for_tokenization_and_hyphenization(self.string, new_entity.string):
                            return

                new_entity.start_char_offset = 0
                new_entity.end_char_offset = 0
                new_entity.vaild = False
                on.common.log.bad_data_long(new_entity.id.split("@")[-2], # language
                                            "name" if "name" in self.__sgml_span_name else "coref",
                                            "couldn't set up character offsets in new tree",
                                            ["document_id", new_entity.id.split("@")[-6]],
                                            ["sentence_index", new_entity.sentence_index],
                                            ["identifier", new_entity.coreference_chain.identifier
                                              if "coref" in self.__sgml_span_name else None],
                                            ["old string", self.string],
                                            ["new string", new_entity.string])
            set_offsets()

        if not same_except_for_tokenization_and_hyphenization(self.string, new_entity.string):
            on.common.log.report("name" if "name" in self.__sgml_span_name else "coref", "not a perfect string match for a span being aligned (for arabic this is almost always true)",
                                 link=self, old_string=self.string, new_string=new_entity.string,
                                 ranked_start_end_candidates=start_end_candidates_ranked)

        assert new_entity.start_leaf.get_token_index() <= new_entity.end_leaf.get_token_index()



        # if the only reason we count as subtoken is because we start
        # or end with punctuation that should not be included, just go
        # ahead and take up the whole token
        if (new_entity.start_char_offset and
            all(c in PUNCT for c in string_without_offsets[:new_entity.start_char_offset])):

            new_entity.start_char_offset = 0
        # also check this for the end of the token
        if (new_entity.end_char_offset and
            all(c in PUNCT for c in string_without_offsets[-new_entity.end_char_offset:])):
    ...
    def enrich_tree(self, a_tree):
        ...
        for a_token in initial_string_tokens:
            for a_leaf in a_tree:
                #
                # get_word(...) is redundant for english and chinese language strings
                #
